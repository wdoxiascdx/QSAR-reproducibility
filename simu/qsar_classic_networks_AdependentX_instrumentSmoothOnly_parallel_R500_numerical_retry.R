...
CFG <- list(
  ...
  # Only the fitted-peer quantile in the instruments is smoothed:
  #   tau_i = 0.160 (n/1000)^(-0.18) (d_i/dbar_n)^(-0.08).
  # There is deliberately no residual/outcome bandwidth in this version.
  ...
  z_grid_size = 81L,
  # Global profile selection with unsmoothed structural Y_-(z) and
  ...
  # numerical z-Jacobians in diagnostics/inference, so a quantile knot gets
...
smooth_instrument_peer_quantile <- function(
    sorted_groups, z, n, mean_degree, cfg = CFG) {
  q <- numeric(n)
  dq <- numeric(n)
  ddq <- numeric(n)

  for (one_group in sorted_groups) {
    tau <- tau_value(n, one_group$degree, mean_degree, cfg)
...
smooth_instrument_peer_quantile_grid <- function(
    sorted_groups, z_grid, n, mean_degree, cfg = CFG) {
...
design_diagnostic <- function(X, network, z0 = NULL, cfg = CFG) {
  ...
  smoothed <- smooth_instrument_peer_quantile(
    sorted_signal,
    z0,
    n,
    mean(network$degree),
...
theorem_inference <- function(
    y, X, outcome_groups, fitted_groups, estimate, network,
    cfg = CFG) {
  ...
  z_hat <- unname(estimate["z"])
  mean_degree <- mean(network$degree)

  # Structural residual: exact linearly interpolated observed-peer quantile.
  # Instrument: Gaussian-smoothed first-stage fitted-peer quantile.
  outcome_at_hat <- interpolated_peer_quantile(
    outcome_groups, z_hat, n
  )
  fitted_at_hat <- smooth_instrument_peer_quantile(
    fitted_groups, z_hat, n, mean_degree, cfg
  )

  M <- cbind(
    X,
    fitted_q = lambda_hat * fitted_at_hat$q,
    fitted_dq = lambda_hat * fitted_at_hat$dq
  )
  residual <- y -
    as.numeric(X %*% beta_hat) -
    lambda_hat * outcome_at_hat$q
  psi <- M * residual
  moment <- colMeans(psi)
  ...
  # beta and lambda columns are analytic.  The z column is a symmetric
  # difference quotient of the complete feasible moment.  This is deliberate:
  # without residual smoothing Y_-(z) is only piecewise differentiable.  Away
  # from a knot this recovers the local slope; at a knot it supplies a stable
  # symmetric local derivative without pretending a classical derivative
  # exists there.
  G_beta <- -crossprod(M, X) / n

  dM_dlambda <- matrix(0, nrow = n, ncol = p + 2L)
  dM_dlambda[, p + 1L] <- fitted_at_hat$q
  dM_dlambda[, p + 2L] <- fitted_at_hat$dq
  G_lambda <- colMeans(
    sweep(dM_dlambda, 1L, residual, "*") -
      sweep(M, 1L, outcome_at_hat$q, "*")
  )

  moment_at_z <- function(z) {
    outcome_one <- interpolated_peer_quantile(outcome_groups, z, n)
    fitted_one <- smooth_instrument_peer_quantile(
      fitted_groups, z, n, mean_degree, cfg
    )
    M_one <- cbind(
      X,
      fitted_q = lambda_hat * fitted_one$q,
      fitted_dq = lambda_hat * fitted_one$dq
    )
    residual_one <- y - as.numeric(X %*% beta_hat) -
      lambda_hat * outcome_one$q
    colMeans(M_one * residual_one)
  }
  z_step <- cfg$jacobian_z_step
  G_z <- (
    moment_at_z(z_hat + z_step) -
      moment_at_z(z_hat - z_step)
  ) / (2 * z_step)
...
refine_profile_basin <- function(
  ...
  if (upper - lower > 1e-10) {
    ...
    # It only evaluates the objective, so kinks created by the unsmoothed
    # structural peer quantile do not cause an invalid derivative call.
    optimized <- tryCatch(
      stats::optimize(
...
estimate_qsar <- function(y, X, network, cfg = CFG) {
  ...
  # The residual uses the raw linearly interpolated Y_-(z).  Only the fitted
  # peer quantile entering the instruments is Gaussian-smoothed with tau_i.
  # Hence the profile can have kinks at empirical-quantile knots.
  outcome_grid <- interpolated_peer_quantile_grid(
    outcome_groups, z_grid, n
  )
  fitted_grid <- smooth_instrument_peer_quantile_grid(
...
sample_jacobian_diagnostic <- function(
  ...
  base_fitted <- smooth_instrument_peer_quantile(
  ...
  secant_diagnostics <- lapply(
    ...
    function(z_index) {
      ...
      for (candidate_index in seq_along(candidate_z)) {
        ...
        fitted_smooth <- smooth_instrument_peer_quantile(
...
validate_tau_bandwidth <- function(network, n, cfg = CFG) {
  ...
  # Effective two-sided interval count for instrument smoothing only.
...
write_outputs <- function(results, cfg = CFG) {
  output_dir <- if (is.null(cfg$output_dir)) {
    sprintf(
      paste0(
        ...
        "instrumentSmoothOnly_numericalRetry_results_",
        "R%d_z030507_cp%d"
      ),
      as.integer(cfg$R),
  ...
  message(sprintf(
    paste0(
      "Global unsmoothed-residual profile selection retained %d candidate ",
...
run_internal_unit_tests <- function(cfg = CFG) {
  ...
  message(
    paste0(
      ...
      "unsmoothed-residual profile multistart, classical-network, progress, ",
...
run_simulation <- function(cfg = CFG) {
  ...
  message(sprintf(
    paste0(
      "Estimator root selection uses unsmoothed structural Y_-(z) and ",
