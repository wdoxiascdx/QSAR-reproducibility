CFG <- list(
  ...
  y_sheet = "Sheet2",
  edge_sheet = "Sheet3",
  output_dir = "qsar_realdata_raw_gmm_gamma50_output",
...
gmm_score_covariance <- function(scores, eigen_floor) {
  ...
  # the average outer product of individual moment scores, without network-lag
  # cross-products.  It permits heteroskedasticity across observations.
...
indexed_moment_components <- function(theta, model) {
  ...
  instruments <- cbind(model$X, peer$fitted, derivative_instrument)
...
fit_indexed_gmm <- function(model, cfg) {
  k <- model$p + 2L
  W <- diag(k)
  ...
  cat(sprintf(
    ...
    model$name, model$index_name, tail(current$theta, 1L),
...
write_outputs <- function(
  ...
  diagnostics <- data.frame(
    ...
    qsar_raw_jacobian_smin = qsar_fit$jacobian_diagnostics$raw_smin,
    qsar_scaled_jacobian_smin = qsar_fit$jacobian_diagnostics$scaled_smin,
  ...
  cat(
    paste0(
      ...
      "GMM sandwich (no network-lag covariance terms).\n"
    )
  )
  cat(sprintf(
    paste0(
...
main <- function(cfg = CFG) {
  ...
  cat("Deleting isolates and retaining degree-one nodes as peer-support alters...\n")
  ...
  cat("Estimating double-smoothed QSAR with the single fixed bandwidth rule...\n")
  qsar_fit <- fit_indexed_gmm(qsar_model, cfg)
...
equivalent_z <- results$alignment$node_level$ces_equivalent_z
equivalent_z <- equivalent_z[is.finite(equivalent_z)]

ces_mean <- results$alignment$summary$ces_equivalent_z_mean
ces_median <- results$alignment$summary$ces_equivalent_z_median
qsar_z <- results$alignment$summary$qsar_z

if (length(equivalent_z) < 2L) {
  stop("Too few finite CES-equivalent quantiles to draw the distribution.")
}

plot_file <- file.path(
  results$output_dir,
  "ces_equivalent_z_histogram_density.png"
)
...
hist(
  ...
  border = "white",
  main = "A. Histogram",
  xlab = "CES-equivalent quantile",
  ylab = "Density"
)

abline(v = ces_mean, col = reference_colors[["CES mean"]], lwd = 2)
abline(v = ces_median, col = reference_colors[["CES median"]], lwd = 2, lty = 2)
abline(v = qsar_z, col = reference_colors[["QSAR z"]], lwd = 2, lty = 3)

legend(
  "topright",
  legend = c(
    sprintf("CES mean = %.3f", ces_mean),
    sprintf("CES median = %.3f", ces_median),
    sprintf("QSAR z = %.3f", qsar_z)
  ),
  col = reference_colors,
  lty = c(1, 2, 3),
  lwd = 2,
  bty = "n",
  cex = 0.85
...
polygon(
  density_result$x,
  density_result$y,
  col = grDevices::adjustcolor("#6BAED6", alpha.f = 0.45),
  border = NA
)
...
cat("\nCES-equivalent quantile distribution plot saved to:\n")
cat(plot_file, "\n")
